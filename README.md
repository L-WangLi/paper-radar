...
- **LLM 一句话总结** — 通过 GitHub Models（免费，无需额外配置）为论文生成中文总结，使用稳定文献 ID 继承已有结果；折叠态卡片和微信推送都直接显示，未生成时自动降级为摘要关键句
- **阅读状态** — 每篇论文可在「待读 → 阅读中 → 已读」之间流转，并记录状态历史
- **个人笔记** — 每篇论文可添加阅读笔记，手动保存，显示保存时间
- **已标记 tab** — 独立查看所有已标记论文，按标记时间排序
- **收藏夹** — 独立 tab 查看收藏论文
- **我的文献库** — 显式加入论文，支持卡片与按入库时间分组的时间线视图；收藏、笔记和阅读标记也会自动沉淀入库
- **稳定文献身份** — 优先使用 DOI、其次 arXiv ID、最后使用规范化标题，避免跨数据源更新后个人状态失联
- **Obsidian 阅读笔记** — 按固定文件名和 YAML 字段创建结构化笔记，并保存可再次打开的本地 OB 链接
- **Zotero 安全入口** — 支持复制 DOI、下载 RIS、打开原文及关联 Zotero item key；不在公开前端保存 Zotero API Key
- **每周回顾** — 汇总本周加入、读完和记录的思考数量，并显示当前阅读进度
- **BibTeX 导出** — 一键复制论文引用格式
- **云同步** — 通过私有 GitHub Gist 备份/恢复笔记、收藏、阅读状态和本地文献关联（跨设备）
